...
class ClientRequestManager
{
    ...
    #_makeFilter(req)
    {
        ...
        if(req.context === true && req.time === false)
            return {sens_id : ObjectID.createFromHexString(req.arg.sens_id)};
        if(req.context === false && req.time === false)
            return {};
    }
    #_sortTemp(a, b)
    {
        ...
        switch(req.type)
        {
            case "temp" : 
                ...
                dbm.readStat(filter, projection, "tempHum", req.param, this.#_sortTemp).then( 
                    (queryRes) => queryRes ?
                         this.#sendResponse(res, 200, req.param + " temperature", JSON.stringify(queryRes))
            ...
            case "hum" :
                ...
                dbm.readStat(filter, projection, "tempHum", req.param, this.#_sortHum).then( 
                    (queryRes) => queryRes ?
                         this.#sendResponse(res, 200, req.param + " humidity", JSON.stringify(queryRes))
                            : this.#sendResponse(res, 400, "Statistic not found", {}));
                break;
            case "lum" :
                console.log("LUM CASE");
                projection = {projection : {_id : 0, lum: 1, sens_id : 1, measureTime : 1}};
                dbm.readStat(filter, projection, "luminosity", req.param, this.#_sortLum).then( 
                    (queryRes) => queryRes ?
                         this.#sendResponse(res, 200, req.param + " luminosity", JSON.stringify(queryRes))
                            : this.#sendResponse(res, 400, "Statistic not found", {}));
                break;
            case "presence" :
                ...
                dbm.readStat(filter, projection, "presence", req.param, this.#_sortPrTime).then( 
                    (queryRes) => queryRes ?
                         this.#sendResponse(res, 200, req.param + " presence", JSON.stringify(queryRes))
                            : this.#sendResponse(res, 400, "Statistic not found", {}));
                break;
            default : 
